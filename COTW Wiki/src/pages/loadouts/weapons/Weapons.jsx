...
export default function Weapons() {
  ...
  // --- DATA ARRAYS (PRESERVING ALL ENTRIES) ---
  const rifleImages = [
  ...
  const rifleStats = [
    ...
    { name: "M1 Iwaniec", class: "4-8", accuracy: 35, recoil: 82, reload: 49, hipshot: 50, mag: 5 },
    ...
    { name: "Curman .50 Inline", class: "4-8", accuracy: 30, recoil: 70, reload: 5, hipshot: 15, mag: 1 },
    { name: "Tsurugi LRR .338", class: "7-9", accuracy: 38, recoil: 100, reload: 49, hipshot: 12, mag: 5 },
    { name: "Malmer 7mm Magnum", class: "4-9", accuracy: 35, recoil: 85, reload: 40, hipshot: 25, mag: 3 },
  ...
  return (
    <div className="wiki-page">
      <div className="wiki-inner">
        {/* HEADER */}
        <h1 className="wiki-header">Weapons</h1>
        ...
        <div className="wiki-layout">
            ...
            <div className="wiki-toc-panel">
              ...
              {isTocOpen && (
                <ul className="wiki-list-plain">
                  <li><a href="#rifles" onClick={(e) => scrollTo(e, 'rifles')} className="wiki-link">1. Rifles</a></li>
                  <li><a href="#shotguns" onClick={(e) => scrollTo(e, 'shotguns')} className="wiki-link">2. Shotguns</a></li>
                  <li><a href="#bows" onClick={(e) => scrollTo(e, 'bows')} className="wiki-link">3. Bows</a></li>
                  <li><a href="#handguns" onClick={(e) => scrollTo(e, 'handguns')} className="wiki-link">4. Handguns</a></li>
                </ul>
              )}
            </div>
          </aside>
          ...
          <main className="wiki-main">
            ...
            {/* Infobox */}
            <div className="float-right w-auto ml-6 mb-4 bg-[var(--wiki-bg-sidebar)] border border-[var(--wiki-border)] shadow-xl p-1">
              <div className="bg-[var(--wiki-bg-sidebar-header)] text-white text-center font-bold py-2 border-b border-[var(--wiki-accent-gold)]">
                Weapons
              </div>
              <div className="p-4 bg-[var(--wiki-bg-sidebar-image)]">
                <img src={WeaponsImage} alt="Weapons" style={{ width: '268.4px', height: '150.32px', objectFit: 'contain' }} />
              </div>
            </div>
            ...
            <div className="wiki-p-mb">
              <p className="mb-4">
                <strong className="text-white">Weapons</strong> are used to hunt.
              </p>
              <p>
                Currently, 4 different weapon classes are available: rifles, handguns, shotguns, and bows (including crossbows). Each class of weapons has unique traits and <a href="#" className="wiki-link">Ammunition</a>, thus requires a different game style. Many of the weapons have variants, which are essentially the same weapon with a different aesthetic.
              </p>
            </div>

            {/* 1. RIFLES */}
            <section id="rifles" className="clear-both pt-8 mb-20">
              <h2 className="wiki-h2">1. Rifles</h2>
              <p className="wiki-p-mb">
                Rifles are the favorite option for many hunters. Provided with scopes, they can be very effective in long-range shooting. From the <Link to="/weapons/rifles/223-docent" className="wiki-link">.223 Docent</Link> for hunting <a href="#" className="wiki-link">foxes</a> and <a href="#" className="wiki-link">coyotes</a>, to the <Link to="/weapons/rifles/quist-reaper-7-62x39" className="wiki-link">Quist Reaper 7.62x39</Link>, and on to the powerful <Link to="/weapons/rifles/rangemaster-338" className="wiki-link">Rangemaster 338</Link> for <a href="#" className="wiki-link">Moose</a> and <a href="#" className="wiki-link">European Bison</a>.
              </p>
                ...
                {rifleImages.map((rifle, idx) => (
                  <div key={idx} className="wiki-gallery-item">
                    <div className="wiki-gallery-placeholder">
                      <img src={rifle.img} alt={rifle.name} />
                    </div>
                    <Link to={rifle.link} className="wiki-gallery-label wiki-link font-bold text-center block mt-2">{rifle.name}</Link>
                  </div>
                ))}
              </div>
                ...
                <table className="wiki-table">
                  <thead>
                    <tr className="bg-[var(--wiki-bg-sidebar-header)]">
                      <th className="wiki-th">Weapon</th>
                      <th className="wiki-th">Weapon Class</th>
                      <th className="wiki-th">Accuracy*</th>
                      <th className="wiki-th">Recoil</th>
                      <th className="wiki-th">Reload Speed*</th>
                      <th className="wiki-th">Hipshot</th>
                      <th className="wiki-th">Magazine Size</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rifleStats.map((rifle, idx) => (
                      <tr key={idx}>
                        <td className="wiki-td"><Link to={rifle.link || "#"} className="wiki-link">{rifle.name}</Link></td>
                        <td className="wiki-td">{rifle.class}</td>
                        <td className="wiki-td text-center">{rifle.accuracy}</td>
                        <td className="wiki-td text-center">{rifle.recoil}</td>
                        <td className="wiki-td text-center">{rifle.reload}</td>
                        <td className="wiki-td text-center">{rifle.hipshot}</td>
                        <td className="wiki-td text-center">{rifle.mag}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <p className="wiki-fs-sm mt-4">* Higher value is better. </p> <p className="wiki-fs-sm mt-2">** Ammunition from another weapon class</p>
            </section>

            {/* 2. SHOTGUNS */}
            <section id="shotguns" className="clear-both pt-8 mb-20">
              <h2 className="wiki-h2">2. Shotguns</h2>
              <p className="wiki-p-mb">
              ...
              <div className="wiki-gallery-grid">
                {shotgunImages.map((gun, idx) => (
                  <div key={idx} className="wiki-gallery-item">
                    <div className="wiki-gallery-placeholder">
                      <img src={gun.img} alt={gun.name} />
                    </div>
                    <Link to={gun.link} className="wiki-gallery-label wiki-link font-bold text-center block mt-2">{gun.name}</Link>
                  </div>
                ))}
              </div>
                ...
                <table className="wiki-table">
                  ...
                  <tbody>
                    {shotgunStats.map((gun, idx) => (
                      <tr key={idx}>
                        <td className="wiki-td">{gun.name}</td>
                        <td className="wiki-td">{gun.class}</td>
                        <td className="wiki-td text-center">{gun.accuracy}</td>
                        <td className="wiki-td text-center">{gun.recoil}</td>
                        <td className="wiki-td text-center">{gun.reload}</td>
                        <td className="wiki-td text-center">{gun.hipshot}</td>
                        <td className="wiki-td text-center">{gun.mag}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {/* 3. BOWS */}
            <section id="bows" className="clear-both pt-8 mb-20">
              <h2 className="wiki-h2">3. Bows</h2>
              <p className="wiki-p-mb">
              ...
              <div className="wiki-gallery-grid">
                {bowImages.map((bow, idx) => (
                  <div key={idx} className="wiki-gallery-item">
                    <div className="wiki-gallery-placeholder">
                      <img src={bow.img} alt={bow.name} />
                    </div>
                    <Link to={bow.link} className="wiki-gallery-label wiki-link font-bold text-center block mt-2">{bow.name}</Link>
                  </div>
                ))}
              </div>
                ...
                <table className="wiki-table">
                  <thead>
                    <tr className="bg-[var(--wiki-bg-sidebar-header)]">
                      ...
                      <th className="wiki-th">Recoil</th>
                      <th className="wiki-th">Reload Speed*</th>
                      <th className="wiki-th">Peak Draw Weight*</th>
                      <th className="wiki-th">Magazine Size</th>
                    </tr>
                  </thead>
                  <tbody>
                    {bowStats.map((bow, idx) => (
                      <tr key={idx}>
                        <td className="wiki-td">{bow.name}</td>
                        <td className="wiki-td">{bow.class}</td>
                        <td className="wiki-td text-center">{bow.accuracy}</td>
                        <td className="wiki-td text-center">{bow.recoil}</td>
                        <td className="wiki-td text-center">{bow.reload}</td>
                        <td className="wiki-td text-center">{bow.draw}</td>
                        <td className="wiki-td text-center">{bow.mag}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <p className="wiki-fs-sm mt-4">* Higher value is better. </p>
            </section>

            {/* 4. HANDGUNS */}
            <section id="handguns" className="clear-both pt-8 mb-20">
              <h2 className="wiki-h2">4. Handguns</h2>
              <p className="wiki-p-mb">
              ...
              <div className="flex flex-wrap items-center gap-x-8 mt-4">
                <p className="wiki-fs-sm m-0">* Higher value is better.</p>
                <p className="wiki-fs-sm m-0">** Ammunition from another weapon class</p>
              </div>
            </section>
